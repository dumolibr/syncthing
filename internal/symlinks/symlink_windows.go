...
func init() {
	if os.Getenv("STNOLINKS") != "" {
		return
	}
	// Needs administrator priviledges.
	// Let's check that everything works, before we make assumptions
	base := os.TempDir()
...
func Read(path string) (string, uint32, error) {
	...
	handle, err := syscall.CreateFile(ptr, 0, 0, nil, syscall.OPEN_EXISTING, syscall.FILE_FLAG_BACKUP_SEMANTICS|FILE_FLAG_OPEN_REPARSE_POINT, 0)
...
func Create(source, target string, flags uint32) error {
	...
	// whether it's a directory symlink or a file link.
	// If the flags doesn't reveal the target type, try to evaluate the it
	// ourselves, and worst case default to the link being a file.
	mode := 0
